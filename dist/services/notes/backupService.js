...
const BACKUP_PATH = process.env.BACKUP_DIRECTORY || path_1.default.join(__dirname, '../../../backups');
const MONGO_URI = process.env.MONGO_URI || '';
const BACKUP_RETENTION_DAYS = 7;
// 디렉토리 확인 및 생성
if (!fs_1.default.existsSync(BACKUP_PATH)) {
    fs_1.default.mkdirSync(BACKUP_PATH, { recursive: true });
}
// 구글 드라이브 API 설정
...
const createGoogleDriveFolder = async (parentFolderId, folderName) => {
    ...
    if (!folderId)
        throw new Error(`폴더 ID 누락: ${folderName}`);
...
const cleanupOldBackups = async (folderPath, googleDriveFolderId) => {
    const files = fs_1.default.readdirSync(folderPath).filter(file => file.endsWith('.json'));
...
const uploadToGoogleDrive = async (filePath, folderId) => {
    ...
    if (!fileId)
        throw new Error('업로드 결과에 fileId가 없습니다.');
    console.log(`구글 드라이브 업로드 성공: ${fileId}`);
};
// 백업 실행
const backupDatabase = async () => {
    const backupConnection = mongoose_1.default.createConnection(MONGO_URI);
    try {
        console.log('백업용 MongoDB 연결 성공');
        const date = new Date().toISOString().replace(/T/, '_').replace(/:/g, '-').split('.')[0];
        const models = { Note: note_1.default, User: user_1.default, Category: category_1.default, Tag: tag_1.default };
        for (const [name, model] of Object.entries(models)) {
            const localFolderPath = path_1.default.join(BACKUP_PATH, name);
            ...
            const backupFilePath = path_1.default.join(localFolderPath, `${name}_${date}.json`);
            fs_1.default.writeFileSync(backupFilePath, JSON.stringify(data, null, 2), 'utf8');
