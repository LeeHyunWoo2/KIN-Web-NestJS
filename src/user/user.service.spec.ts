...
describe('UserService', () => {
  ...
  describe('resetPassword', () => {
    ...
    const hashedPassword = 'hashed-NewPass123!';
    const now = new Date();
    ...
    it('유저가 존재하지 않으면 예외를 던져야 합니다', async () => {
      ...
      const input: ResetPasswordInput = { email, newPassword };
      await expect(userService.resetPassword(input)).rejects.toThrow(UserNotFoundException);
    });

    it('현재 비밀번호와 같으면 SamePasswordUsedException을 던져야 합니다', async () => {
    ...
    it('과거에 사용한 비밀번호면 PasswordReusedException을 던져야 합니다', async () => {
      const user = {
    ...
    it('정상 요청이면 비밀번호가 해싱되고 저장되어야 합니다', async () => {
    ...
    it('패스워드 히스토리는 최대 5개까지만 유지되어야 합니다', async () => {
      const persist = jest.fn();
      const history = Array.from({ length: 6 }, (_, i) => ({
        password: `old-${i}`,
        changedAt: new Date(now.getTime() - i * 86400_000),
      }));
      const user = {
        id: 1,
        email,
      ...
      expect(user.passwordHistory.length).toBe(5);
      expect(user.passwordHistory.some((h) => h.password === 'old-0')).toBe(false);
      expect(persist).toHaveBeenCalled();
    });
  });
